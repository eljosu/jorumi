...
exports.validateMoveCharacter = validateMoveCharacter;
exports.validateGatherResources = validateGatherResources;
exports.validateBuildStructure = validateBuildStructure;
exports.validateHealWounded = validateHealWounded;
exports.validateTransferResources = validateTransferResources;
exports.validateAttackAlien = validateAttackAlien;
exports.validateAlienControlGhetto = validateAlienControlGhetto;
exports.validateActivateBeacon = validateActivateBeacon;
 ...
 */
function validateGameNotOver(state) {
    if (state.gameOver) {
 ...
 */
function validatePlayerTurn(state, playerId) {
 ...
 */
function validatePhase(state, requiredPhase) {
 ...
 */
function validateMoveCharacter(state, action) {
    if (action.type !== types_2.ActionType.MOVE_CHARACTER) {
        return { valid: false, reason: 'Invalid action type' };
    }
    // Verificar fase
    const phaseCheck = validatePhase(state, types_1.GamePhase.MOVEMENT);
    if (!phaseCheck.valid)
        return phaseCheck;
    ...
    if (currentTileId) {
        ...
        if (currentTile) {
            const distance = (0, hex_1.hexDistance)(currentTile.coordinates, targetTile.coordinates);
            if (distance > constants_1.MOVEMENT_RULES.CHARACTER_MOVE_RANGE) {
                return { valid: false, reason: `Movement range exceeded. Max: ${constants_1.MOVEMENT_RULES.CHARACTER_MOVE_RANGE}, Attempted: ${distance}` };
            }
        }
    }
    return { valid: true };
 ...
 */
function validateGatherResources(state, action) {
    if (action.type !== types_2.ActionType.GATHER_RESOURCES) {
        return { valid: false, reason: 'Invalid action type' };
    }
    // Verificar fase
    const phaseCheck = validatePhase(state, types_1.GamePhase.RESOURCE_GATHERING);
    if (!phaseCheck.valid)
        return phaseCheck;
    // Verificar que el personaje existe
    const character = state.characters.get(action.characterId);
    ...
    const capacity = constants_1.CHARACTER_GATHERING_CAPACITY[character.type];
    ...
    const maxAmount = capacity[action.resourceType] ?? 0;
 ...
 */
function validateBuildStructure(state, action) {
    if (action.type !== types_2.ActionType.BUILD_STRUCTURE) {
        return { valid: false, reason: 'Invalid action type' };
    }
    // Verificar fase (construcción puede ocurrir en fase de comercio)
    const phaseCheck = validatePhase(state, types_1.GamePhase.TRADING);
    if (!phaseCheck.valid)
        return phaseCheck;
    ...
    if (ghetto.controlStatus === types_1.GhettoControlStatus.ALIEN) {
        return { valid: false, reason: 'Cannot build in alien-controlled ghetto' };
    }
    // Verificar que no se excede el límite de edificios
    if (ghetto.buildings.length >= constants_1.GAME_LIMITS.MAX_BUILDINGS_PER_GHETTO) {
        return { valid: false, reason: `Maximum buildings reached (${constants_1.GAME_LIMITS.MAX_BUILDINGS_PER_GHETTO})` };
    }
    // Verificar que el guetto tiene los recursos necesarios
    const cost = constants_1.BUILDING_COSTS[action.buildingType];
    if (!(0, helpers_1.hasEnoughResources)(ghetto.resources, cost)) {
 ...
 */
function validateHealWounded(state, action) {
    if (action.type !== types_2.ActionType.HEAL_WOUNDED) {
        return { valid: false, reason: 'Invalid action type' };
    }
    // Verificar fase
    const phaseCheck = validatePhase(state, [types_1.GamePhase.RESOURCE_GATHERING, types_1.GamePhase.TRADING]);
    if (!phaseCheck.valid)
        return phaseCheck;
 ...
 */
function validateTransferResources(state, action) {
    if (action.type !== types_2.ActionType.TRANSFER_RESOURCES) {
        return { valid: false, reason: 'Invalid action type' };
    }
    // Verificar fase
    const phaseCheck = validatePhase(state, types_1.GamePhase.TRADING);
    ...
    if (!(0, helpers_1.hasEnoughResources)(fromGhetto.resources, action.resources)) {
 ...
 */
function validateAttackAlien(state, action) {
    if (action.type !== types_2.ActionType.ATTACK_ALIEN) {
        return { valid: false, reason: 'Invalid action type' };
    }
    // El ataque puede ocurrir en varias fases
    const phaseCheck = validatePhase(state, [types_1.GamePhase.MOVEMENT, types_1.GamePhase.ALIEN_TURN]);
    if (!phaseCheck.valid)
        return phaseCheck;
    ...
    const distance = (0, hex_1.hexDistance)(characterTile.coordinates, alienTile.coordinates);
    if (distance > constants_1.MOVEMENT_RULES.ADJACENT_DISTANCE) {
 ...
 */
function validateAlienControlGhetto(state, action) {
    if (action.type !== types_2.ActionType.ALIEN_CONTROL_GHETTO) {
        return { valid: false, reason: 'Invalid action type' };
    }
    // Verificar fase
    const phaseCheck = validatePhase(state, types_1.GamePhase.ALIEN_TURN);
    if (!phaseCheck.valid)
        return phaseCheck;
    // Verificar que el guetto existe
    const ghetto = state.ghettos.get(action.ghettoId);
 ...
 */
function validateActivateBeacon(state, action) {
    if (action.type !== types_2.ActionType.ACTIVATE_BEACON) {
        return { valid: false, reason: 'Invalid action type' };
    }
    // Verificar que el guetto existe
    const ghetto = state.ghettos.get(action.ghettoId);
    ...
    if (!ghetto.buildings.includes(types_1.BuildingType.BEACON)) {
